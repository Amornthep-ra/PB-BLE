...
#else
  log_v("--- dumpInfo ---");
  log_v("ESP32-only info not available on this target.");
  log_v("---");
#endif
}  // dumpInfo
...
#else
  (void)errCode;
  return "ESP_ERR unsupported on this target";
#endif
}  // errorToString
...
#else
  (void)errCode;
  return "WiFi errors unsupported on this target";
#endif
}  // wifiErrorToString
...
String GeneralUtils::toLower(String &value) {
  ...
  for (size_t i = 0; i < value.length(); ++i) {
    value.setCharAt(i, (char)tolower((unsigned char)value[i]));
  }
  return value;
}  // toLower
