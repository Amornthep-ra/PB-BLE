...
BLEEddystoneURL::BLEEddystoneURL(BLEAdvertisedDevice *advertisedDevice) {
  ...
  for (int i = 0; i < advertisedDevice->getPayloadLength(); ++i) {
    if (payload[i] == 0x16 && advertisedDevice->getPayloadLength() >= i + 2 + sizeof(m_eddystoneData) && payload[i + 1] == 0xAA && payload[i + 2] == 0xFE
      ...
      if (lengthURL <= 18) {
        setData(bytesToString((uint8_t *)(payload + i + 4), lengthURL + 1));
      } else {
...
String BLEEddystoneURL::getData() {
  return bytesToString((uint8_t *)&m_eddystoneData, sizeof(m_eddystoneData));
}  // getData
...
String BLEEddystoneURL::getFrame() {
  ...
  String frame = bytesToString((uint8_t *)BLEHeadder, sizeof(BLEHeadder));
  frame += bytesToString((uint8_t *)&m_eddystoneData, lengthURL + 1);  // + 1 for TX power
...
String BLEEddystoneURL::getURL() {
  return bytesToString((uint8_t *)&m_eddystoneData.url, lengthURL);
}  // getURL
...
int BLEEddystoneURL::setSmartURL(String url) {
  ...
  for (size_t i = 0; i < url.length(); ++i) {
    ...
    if (!isPrintable((unsigned char)character)) {
